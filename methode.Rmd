...
::: {align="justify"}
La formation s'appuie sur une méthode et une progression pédagogique
conçues pour les débutant-e-s, et des méthodes basée sur les acquis des
sciences cognitives en matière d'apprentissage pour renforcer la
mémorisation. Elle utilise l'interface conviviale de RStudio, qui
facilite l'utilisation de R, le format Rmarkdown, qui permet de générer
automatiquement des rapports (html,word, pdf ou même des diaporamas) à
partir du code, des quiz interactifs générés avec le package learnr et
une application interactive pour l'autoévaluation des acquis générée
avec le package shiny. Grace à tout cela, les participant-e-s sont
étonné-e-s de ce qu'ils-elles sont capables de faire à la fin du
deuxième jour!
:::
...
de situations de recherche réelles en écologie et agronomie, permettant
une immersion dans la formation plus aisée pour les chercheur-e-s en
sciences de la vie, pour qui cette formation est conçue.
:::
...
-   La réactivation mémoire (i) lors des quiz, (ii) lors d'un exercice
    de lecture de code en ouverture du deuxième jour, qui permet de
    réviser les fonctions de base vues lors de la séquence 2, et (iii)
    par l'intégration de "goodies" dans chaque séquence (c'est à dire
    des fonctions et astuces non couvertes pendant la formation,
    simplement mentionnées mais écrites dans le document donné aux
    participant-e-s) que les participant-e-s devront découvrir
    d'eux-elles mêmes après la formation, afin de les inciter à la
    réviser pour renforcer la mémorisation.
:::
...
Figure 4: Les points forts de la formation: (A) une formation
progressive (réutilisation et exploration progressive des fonctions de
la séquence 2 et (D) exercices de plus en plus longs et complexes ), (B)
des séquences enchainant théorie/quiz/exercices pour renforcer
l'apprentissage, (C) une méthode pédagogique adaptées aux débutant-e-s,
(E) des tableaux issus de situations réelles de recherche en sciences de
la vie, et (F) un ensemble de méthodes et outils pour renforcer la
mémorisation.
...
contextes différents), générés automatiquement par une application
shiny. Le tableau est alors exportable et le code permettant de générer
les graphiques intégré au document rmarkdown de la formation sous forme
de goodies.
:::

## Architecture de la formation

La formation repose sur 4 documents utilisés par les participant-e-s
(fig. 5):
...
-   Une application shiny permettant de générer les graphiques de
    l'autoévaluation et d'exporter le tableau des résultats
